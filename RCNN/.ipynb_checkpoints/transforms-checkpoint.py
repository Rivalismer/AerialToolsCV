...
class Resize(nn.Module):
    ...
    def forward(self, img: Tensor, target: Optional[Dict[str, Tensor]]
    ) -> Tuple[Tensor, Optional[Dict[str, Tensor]]]:
        image = F.resize(img, self.size, interpolation = InterpolationMode.NEAREST)
        target["boxes"][:, 0::2] *= self.size[0] / self.orig_w
        target["boxes"][:, 1::2] *= self.size[1] / self.orig_h
        if "masks" in target:
            target["masks"] = F.resize(
                target["masks"], self.size, interpolation=InterpolationMode.NEAREST
            )
        return image, target
